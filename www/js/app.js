...
.run(function($ionicPlatform, $cordovaSQLite) {
  $ionicPlatform.ready(function() {
	...
	// window.openDB is used on the actual phone, window.openDatabase is chrome Syntax, to be able to use the "ionic serve" command to run the app in google chrome browser on pc
		...
		$cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS Tipprunde(tipprunde_id INTEGER PRIMARY KEY, tipprunde_name TEXT UNIQUE, tipprunde_passwort TEXT, tipprunde_europameisterschaft_fid INTEGER NOT NULL, FOREIGN KEY(tipprunde_europameisterschaft_fid) REFERENCES Europameisterschaft(europameisterschaft_id))");
		...
		$cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS Tipp(begegnung_fid INTEGER NOT NULL, benutzer_fid INTEGER NOT NULL, tipprunde_fid INTEGER NOT NULL, tipp_tore_heimmannschaft INTEGER, tipp_tore_auswaertsmannschaft INTEGER, tipp_datum TIMESTAMP DEFAULT CURRENT_TIMESTAMP, status TEXT, PRIMARY KEY(begegnung_fid, benutzer_fid, tipprunde_fid), FOREIGN KEY(begegnung_fid) REFERENCES Begegnung(begegnung_id), FOREIGN KEY(benutzer_fid) REFERENCES Benutzer(benutzer_id), FOREIGN KEY(tipprunde_fid) REFERENCES Tipprunde(tipprunde_id))");
